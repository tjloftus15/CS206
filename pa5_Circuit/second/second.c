...
int main(int argc, char ** argv){
	...
	actionCodes=(int *)malloc(fileLength*sizeof(int));//codes for each line on the action to take
	/*vars[actionLines]=(int*)malloc(sizeof(int));
	vars[actionLines][0]=-1;
	vars[actionLines+1]=(int*)malloc(sizeof(int));
	vars[actionLines+1][0]=-1;*/
	int * lineOrder=(int*)malloc(fileLength*sizeof(int));
	int varPos=0;
	int numPos=0;
	int outPos=0;
	...
	while(read<fileLength){
		...
		switch(curr){
			...
			case 'M':
				...
				varNumber[numPos]=(int*)malloc(4*sizeof(int));
				i=getFirstInput(line, numInputs);
				numOutputs=LogTwo(numInputs);
				//numOutputs=numOutputs+numInputs;
				varNumber[numPos][2]=numInputs;
				varNumber[numPos][3]=numOutputs;
				numInputs=numInputs+numOutputs;
				varNumber[numPos][0]=numInputs;
				varNumber[numPos][1]=1;
				totalVar=numOutputs+numInputs+1;
				vars[varPos]=(int*)malloc(totalVar*sizeof(int));
				vars[varPos]=getKeys(line, table, i, totalVar, hashSize, numInputs, numOutputs);
				varPos++;
				numPos++;
				break;
			case 'D':
			...
			case 'A':
				...
				vars[varPos]=getKeys(line, table, i, 3, hashSize, numInputs, numOutputs);
				varPos++;
				varNumber[numPos][0]=numInputs;
				varNumber[numPos][1]=numOutputs;
				numPos++;
			...
			case 'N':
				if(line[2]=='T'){
					...
					vars[varPos]=getKeys(line, table, i, 2, hashSize, numInputs, numOutputs);
					varPos++;
					varNumber[numPos][0]=numInputs;
					varNumber[numPos][1]=numOutputs;
					numPos++;
	...
	//order the actions
	int KH=0;
	int * actionOrder=(int*)malloc(fileLength*sizeof(int));
	int * changed=(int*)malloc(fileLength*sizeof(int));
	i=0;
	while(i<fileLength){
		changed[i]=-1;
		actionOrder[i]=-1;
	...
	//int visited=0;
	//int xkey=0;
	int A=0;//value in action array
	int zkey=0;
	//printf("file length: %d\n", fileLength);
	/*while(visited<fileLength){
		printf("%d: %d\n", visited, actionCodes[visited]);
		visited++;
	}*/
	//visited=0;
	KH=0;
	...
	while(times<fileLength){
		...
		while(KH<fileLength){
			if(actionCodes[KH]==-1 || actionCodes[KH]==0){
				actionOrder[A]=-1;
				A++;
				KH++;
				continue;
			...
			inVar=varNumber[KH][0];
			i=0;
			t1=-1;
			while(i<inVar){//searches for in variable that has not yet been out var
				//printf("---------------------------------------------------------------------------\n");
				zkey=vars[KH][i];
				if(zkey==hashSize || zkey==hashSize+1){
					i++;
					continue;
				}
				
				t1=KH;
				if(table[zkey].value==-1){//found an input key that is -1
					//printf("i found -1 key\n");
					//printf("the variable at -1 is %s\n", table[zkey].key);
					akey=zkey;
					i=0;
					while(KH<fileLength){//searching entire vars outs for akey
						//KH=firstChange;
						
						int last=varNumber[KH][0];
						int totalOut=last+varNumber[KH][1];
						//printf("KH=%d actionLines=%d\n", KH, actionLines);
						//printf("ORIG last=%d totalOut=%d\n", last, totalOut);
						if(last==0 || totalOut==0){
							KH=fileLength;
							//printf("in new break\n");
							break;
							
						}
						while(last<totalOut){//searches var outputs for the key i want
							bkey=vars[KH][last];
							//printf("akey = %d bkey = %d\n", akey, bkey);
							//printf("last=%d totalOut=%d\n", last, totalOut);
							if(bkey==hashSize || bkey == hashSize+1){
								last++;
								continue;
							}
							else if(akey==bkey){//found the key i was looking for in outputs of KH last
								t2=KH;//the line that must come before t1;
								table[zkey].value=0;
								//changeMade=1;
								break;
						...
						KH++;
						if(table[zkey].value==-1 && KH==fileLength){
							KH=0;
						}
						if(t2!=-1)
							break;	
	...
	while(runs<totalGray){//this part gets each new gray code  sequence!!!!!/////
		...
		//printf("------------------------------------------------------------------------\n");
			int action=actionCodes[cAct];
			numPos=lineOrder[LO];
			varPos=lineOrder[LO];
			LO++;
			//printf("found action from actionCodes[%d] = %d\n", cAct, action);
			if(action==0){
				break;
			}
			switch(action){
				case 1:
					//printf("M\n");
					numInputs=varNumber[numPos][2];
					numOutputs=varNumber[numPos][3];
					numPos++;
				...
				case 3:
					...
					if(skey>=hashSize)
						skey=skey-hashSize;
					f=table[fkey].value;
					//printf("in NOT: fkey = %s, skey = %s\n", table[fkey].key, table[skey].key);
				...
				case 4:
					...
					if(fkey>=hashSize){
						...
						a=table[fkey].value;
						//printf("IN AND: fkey =%s ", table[fkey].key);
					}if(skey>=hashSize){
						b=skey-hashSize;
					}else{
						b=table[skey].value;
						//printf("skey=%s ", table[skey].key);
					}
					//printf("tkey=%s \n", table[tkey].key);
					c=a&b;
					table[tkey].value=c;
					//printf("%d = %d & %d\n", c, a,b);
					//printf("%d = table[tkey].value\n", table[tkey].value);
				...
				case 5:
					...
					if(fkey>=hashSize){
						...
						a=table[fkey].value;
						//printf("IN OR: fkey =%s ", table[fkey].key);
					}if(skey>=hashSize){
						b=skey-hashSize;
					}else{
						b=table[skey].value;
				...
				case 6:
					...
					if(fkey>=hashSize){
						...
						a=table[fkey].value;
						//printf("IN XOR: fkey =%s ", table[fkey].key);
					}if(skey>=hashSize){
						b=skey-hashSize;
					}else{
						b=table[skey].value;
